 ...
 *
 * Xác minh căn hộ không nằm trong 10 tool của Agent (`tool_contract.py`): nó do
 * một đơn vị độc lập đối chiếu giấy tờ rồi quyết định. Trước đây form này nằm
 * ngay trong workspace của P-118, nên màn hình nói một đằng còn mô hình trách
 * nhiệm là một nẻo — người dùng tưởng P-118 tự duyệt được, và khi Agent trả lời
 * "mình không tự làm thay được" thì câu đó mâu thuẫn với chính giao diện.
 *
 * Giờ nó dùng chung vỏ với `/review`: cùng một cổng, hai vai. Người nộp thấy
 * mình rời khỏi P-118 để làm việc với đơn vị xác thực — đúng như thực tế.
 *
    ...
    <div className="space-y-5">
      <header>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Khai thông tin căn hộ và tải ảnh giấy tờ để đơn vị xác thực đối chiếu. Sau khi được duyệt,
          các dịch vụ dành cho cư dân trên P-118 sẽ mở.
        </p>
      </header>
        ...
        >
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Đơn vị xác thực sẽ đối chiếu với hồ sơ căn hộ. Việc xác nhận do đơn vị thực hiện, không
              tự khai được.
            </p>
          </div>
          ...
          <div>
            ...
            <label className="mt-1 flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-gray-300 bg-white px-3 py-4 text-sm text-gray-500 hover:border-indigo-700 hover:text-indigo-700 dark:border-gray-700 dark:bg-gray-900">
          ...
          <button
            type="submit"
            disabled={
              submitting || !apartment.trim() || !area.trim() || !fullName.trim() || files.length === 0
            }
      ...
      {/* Lối về luôn hiện. Người dùng bị đưa sang một hệ thống khác thì phải
          thấy đường quay lại, nếu không họ chỉ còn nút Back của trình duyệt. */}
      <Link
        to="/apartment-link"
        className="inline-flex items-center gap-1.5 text-sm text-indigo-700 hover:underline dark:text-indigo-300"
      >
