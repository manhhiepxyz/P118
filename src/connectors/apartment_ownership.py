"""ApartmentOwnershipConnector stub cho P-118.

Connector cho Apartment Ownership Service.
Xử lý tool: verify_apartment_ownership
Endpoint: POST /api/apartment-owners/verify-ownership
"""
...
class ApartmentOwnershipConnector(Connector):
    """Connector cho Apartment Ownership Service.
    ...
    def tool_names(self) -> list[str]:
        return ["verify_apartment_ownership"]
    ...
    async def execute(
        ...
        if tool_name != "verify_apartment_ownership":
        ...
        try:
            async with self._get_client() as client:
                response = await client.post(
                    f"{self.base_url}/api/apartment-owners/verify-ownership",
                    json=input_data,
                ...
                if response.is_success:
                    data = response.json()
                    return StandardResult.ok(data=data.get("data", {}))
            ...
            return StandardResult.fail(
                ...
                error_message="Apartment Ownership service timeout",
                retryable=True,
            )
        except httpx.ConnectError:
            return StandardResult.fail(
                error_code=ErrorCode.SERVICE_UNAVAILABLE,
                error_message="Không thể kết nối Apartment Ownership service",
                retryable=True,
            )
    ...
    def _map_error_code(self, code: str) -> ErrorCode:
        ...
        mapping = {
            ...
            "OWNERSHIP_NOT_FOUND": ErrorCode.OWNERSHIP_NOT_FOUND,
            "OWNERSHIP_MISMATCH": ErrorCode.OWNERSHIP_MISMATCH,
