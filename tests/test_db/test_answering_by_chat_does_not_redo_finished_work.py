...
`{"viewing_date": <ngày mới>}` rút từ câu chat có cấu trúc y hệt cái rút từ
...
# NEO VÀO HÔM NAY, không viết cứng.
#
# Bản trước ghi thẳng "2026-08-23" cho ngày giữ chỗ. Nó đúng vào hôm test được
# viết, rồi lịch trôi qua ngày đó và `TaskPlanValidator` bắt đầu từ chối kế
# hoạch vì "booking_date in the past" — nên đường SỬA thất bại và hệ thống rơi
# về lập lại từ đầu. Test đỏ với đúng thông điệp mà nó sinh ra để canh, nhưng
# vì một lý do hoàn toàn khác.
#
# Một test hỏng theo ngày tháng còn tệ hơn không có test: nó đỏ vào một buổi
# sáng bất kỳ, không ai đụng vào mã, và người đọc đi tìm hồi quy không tồn tại.
NGAY_THAM_QUAN = (date.today() + timedelta(days=7)).isoformat()
NGAY_GIU_CHO = (date.today() + timedelta(days=6)).isoformat()
NGAY_DOI_SANG = date.today() + timedelta(days=8)

GOAL = (
    f"Đặt lịch tham quan Vinhomes Green Paradise ngày {NGAY_THAM_QUAN}, "
    f"đăng ký xe và giữ chỗ đỗ xe ngày {NGAY_GIU_CHO}."
)
...
async def _cha_da_chay_mot_nua(pool, owner: str) -> str:
    ...
    await pool.execute(
        ...
        wid,
        json.dumps({"project_id": "PRJ-005", "viewing_date": NGAY_THAM_QUAN, "viewing_time": "10:30"}),
    )
    await pool.execute(
    ...
    await pool.execute(
        ...
        " 'ACKNOWLEDGED')",
        wid,
        json.dumps({"vehicle_id": "VEH-28", "parking_zone": "ZONE_A", "booking_date": NGAY_GIU_CHO}),
...
async def test_finished_services_are_never_run_again(client, db_pool, monkeypatch, cach_tra_loi: str):
    ...
    body = (
        {"message": f"đổi qua ngày {NGAY_DOI_SANG.day}"}
        if cach_tra_loi == "chat"
        else {"fields": {"viewing_date": NGAY_DOI_SANG.isoformat()}}
    )
...
async def test_the_answer_reaches_the_step_that_asked(client, db_pool, monkeypatch):
    ...
    await client.post(
        ...
        json={"message": f"đổi qua ngày {NGAY_DOI_SANG.day}"},
    ...
    assert any(r["ngay"] == NGAY_DOI_SANG.isoformat() for r in ngay), [dict(r) for r in ngay]
