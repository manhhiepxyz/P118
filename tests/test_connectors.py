...
#
# Hai tool này chạy trên implementation cũ (`book_tour`, `register_consultation`).
# Điều các test dưới đây bảo vệ KHÔNG phải là "đổi tên đúng", mà là ranh giới:
# từ vựng nội bộ dừng ở provider, và phút giờ người dùng chọn không bị làm tròn.
# =====================================================================


@pytest.mark.asyncio
async def test_schedule_property_viewing_returns_the_canonical_contract(tour_client):
    connector = TourConnector(client=tour_client)

    result = await connector.execute(
        "schedule_property_viewing",
        {"project_id": "PRJ-001", "viewing_date": "2030-05-05", "viewing_time": "09:30"},
    )
    ...
    # Tên tra từ danh mục canonical (src/common/projects.py), không hardcode
    # ở tour provider — đó là điều giữ search và đặt lịch không lệch nhau.
    assert result.data["project_name"] == project_name("PRJ-001")


@pytest.mark.asyncio
async def test_viewing_keeps_the_exact_minute_the_user_asked_for(tour_client):
    """Giờ hẹn phải quay về nguyên văn.

    Implementation bên dưới chỉ có hai khung MORNING/AFTERNOON. Nếu adapter quy
    09:30 về "MORNING" rồi dựng lại giờ từ khung đó, người dùng sẽ nhận xác nhận
    cho một giờ họ chưa từng chọn. Đó là mất dữ liệu, không phải làm tròn.
    """
    connector = TourConnector(client=tour_client)

    for asked in ("08:15", "11:45", "13:05", "17:50"):
        result = await connector.execute(
            "schedule_property_viewing",
            {"project_id": "PRJ-002", "viewing_date": "2030-06-01", "viewing_time": asked},
        )
        assert result.success is True, result.message
        assert result.data["viewing_time"] == asked


@pytest.mark.asyncio
async def test_internal_tour_vocabulary_never_crosses_the_connector(tour_client):
    connector = TourConnector(client=tour_client)

    result = await connector.execute(
        "schedule_property_viewing",
        {"project_id": "PRJ-001", "viewing_date": "2030-07-07", "viewing_time": "10:00"},
    )

    assert result.success is True, result.message
    leaked = {"tour_id", "tour_date", "tour_slot", "residential_area"} & set(result.data)
    assert not leaked, f"từ vựng nội bộ lọt ra Agent: {sorted(leaked)}"
    # Giá trị id cũng không được mang prefix nội bộ.
    assert not result.data["viewing_id"].startswith("TOUR-")


@pytest.mark.asyncio
async def test_viewing_rejects_a_project_that_is_not_in_the_catalogue(tour_client):
    connector = TourConnector(client=tour_client)

    result = await connector.execute(
        "schedule_property_viewing",
        {"project_id": "PRJ-KHONG-CO", "viewing_date": "2030-05-05", "viewing_time": "09:30"},
    )
...
@pytest.mark.asyncio
async def test_tour_connector_refuses_the_old_tool_name(tour_client):
    """`book_tour` không còn là tool public; gọi nó phải bị từ chối."""
    result = await TourConnector(client=tour_client).execute("book_tour", {})
...
@pytest.mark.asyncio
async def test_register_property_interest_returns_the_canonical_contract(consultation_client):
    connector = ConsultationConnector(client=consultation_client)
    ...
    leaked = {"consultation_id", "consultation_type", "buy_sub_type"} & set(result.data)
    assert not leaked, f"từ vựng nội bộ lọt ra Agent: {sorted(leaked)}"


@pytest.mark.asyncio
async def test_interest_is_refused_without_explicit_consent(consultation_client):
    """Không có đồng ý rõ ràng thì không được ghi nhận đăng ký liên hệ.

    `consent=False` và `consent` thiếu hẳn đều phải trượt — Pydantic coerce khá
    rộng nên chỉ khai báo `bool` là chưa đủ để chặn.
    """
    connector = ConsultationConnector(client=consultation_client)
    base = {"project_id": "PRJ-001", "interest_type": "rent", "preferred_contact_time": "morning"}

    for payload in ({**base, "consent": False}, base):
        result = await connector.execute("register_property_interest", payload)
        assert result.success is False, f"đăng ký lọt qua với payload {sorted(payload)}"


@pytest.mark.asyncio
async def test_consultation_connector_refuses_the_old_tool_name(consultation_client):
    result = await ConsultationConnector(client=consultation_client).execute("register_consultation", {})

    assert result.success is False
    assert result.error_code == ErrorCode.INVALID_INPUT


def test_book_shuttle_is_not_registered_with_the_executor():
    """`book_shuttle` là experimental — Executor không được biết tới nó.

    Source vẫn còn trong repo, nên chỉ đọc code là dễ tưởng tool vẫn chạy được.
    Ràng buộc thật nằm ở registry: nó phải vắng mặt. Bật lại mà chưa đổi
    `tour_id` → `viewing_id` sẽ tạo một tool Planner không cấp input nổi.
    """
    from src.orchestration.deps import build_connectors

    registered = {name for connector in build_connectors() for name in connector.tool_names}

    assert "book_shuttle" not in registered
    assert "schedule_property_viewing" in registered
    assert "register_property_interest" in registered
