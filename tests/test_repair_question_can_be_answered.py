...
def test_a_repair_question_is_persisted_so_it_can_be_answered() -> None:
    ...
    assert "_persist_repair_clarification(" in source, (
        "câu hỏi lại không được ghim thành lượt chờ bổ sung — `/continue` sẽ "
        "trả 409 và người dùng gặp ngõ cụt"
    )
...
def test_the_persisted_question_asks_for_the_field_that_actually_broke() -> None:
    ...
    assert "repair_missing_fields(" in body, (
        "không lấy ô cần sửa từ bộ phân loại lỗi — hỏng khu lại đi hỏi ngày"
    )
    assert "if not fields:" in body, (
        "ghim một lượt chờ KHÔNG có ô nào: `/continue` trả 'không chờ thêm "
        "thông tin' và người dùng vẫn kẹt"
    )
...
def test_it_no_longer_reads_as_a_contradiction() -> None:
    ...
    text = repair_question(
        "book_parking", "BOOKING_ALREADY_EXISTS", {"booking_date": "2026-08-23"}
    )
